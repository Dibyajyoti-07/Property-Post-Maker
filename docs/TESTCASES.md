...
| AI-04 | Text runs on Groq | `askJSON()` calls `api.groq.com/openai/v1/chat/completions` directly from the browser with the key injected at render time — confirmed via direct testing to allow CORS. Theme/color/extraction/planning never prompt any sign-in. |
| AI-05 | Image source: Pollinations, no Puter | Photos are fetched directly from Pollinations' Flux endpoint (`pollinationsUrl()`/`fetchPollinationsImg()`) — no Puter script, no relay, no login. **Not yet verified end-to-end via automated browser testing**: Pollinations' bot-check consistently blocked this project's own CDP-driven automation (a known signal for headless traffic); a normal interactive browser session should behave differently. Verify manually before treating this as confirmed working. |
| AI-06 | Sequential image generation | The four photo fetches run sequentially (not `Promise.all`), each under a 25s timeout, with a status label per call — Pollinations rate-limits anonymous requests to roughly one every 15 seconds, so back-to-back generations may need to wait. |
| AI-07 | One retry on failure | A single failed photo fetch (or a timeout) is retried once automatically (3s backoff) before surfacing a user-facing error bubble. **Regression case**: the error bubble originally showed the literal string "undefined" for non-Error rejection values; fixed to fall back through `err.message` → `err.error` → `JSON.stringify(err)`. |
