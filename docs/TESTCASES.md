...
| AI-04 | Text runs on Groq, not Puter | `askJSON()` calls `api.groq.com/openai/v1/chat/completions` directly from the browser with the key injected at render time — confirmed via direct testing to allow CORS with no Turnstile/bot-check, unlike Pollinations' text API. Theme/color/extraction/planning never prompt a Puter sign-in. |
| AI-05 | Image source: Puter txt2img (not Pollinations) | **Regression history**: Pollinations' Flux endpoint was tried first (free/unlimited/no-login) but its image host 403s any programmatic read (`img.crossOrigin`, `fetch()`) while allowing plain `<img>` embeds — confirmed by testing both directly. Routing through `puter.net.fetch()` (a CORS-bypass relay) worked initially, then started hanging indefinitely session-wide, confirmed with a fresh tab and a trivial unrelated URL. Generic CORS proxies tried as fallback (allorigins.win, corsproxy.io) were unreliable/gated too. Reverted to `puter.ai.txt2img({model:'gpt-image-1'})`, the only path that's been reliable end-to-end — this does mean image generation is metered by Puter's per-account credit allotment again. |
| AI-06 | Sequential image generation | The four `txt2img` calls run sequentially (not `Promise.all`) with a status label per call — parallel calls previously triggered a "Too many concurrent requests" error on the free tier. |
| AI-07 | One retry on failure | A single injected `txt2img`/Groq-fetch failure is retried once automatically before surfacing a user-facing error bubble. **Regression case**: the error bubble originally showed the literal string "undefined" for non-Error rejection values (e.g. a rejected Puter auth promise); fixed to fall back through `err.message` → `err.error` → `JSON.stringify(err)`. |
| AI-08 | First-run Puter auth (images only) | On a visitor's first *image* generation, Puter may show a one-time sign-in popup once the account's credit allotment needs it; generation resumes correctly once signed in. Text generation never triggers this, since it's on Groq. |
